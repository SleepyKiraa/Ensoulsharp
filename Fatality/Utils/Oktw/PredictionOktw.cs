...
namespace Fatality.Utils.Oktw
{
    public class PredictionOktw
    {
        ...
        public static class Prediction
        {
            ...
            public static PredictionOutput GetPrediction(Spell spell,
                AIBaseClient unit,
                bool Aoe = false,
            ...
            internal static PredictionOutput GetPrediction(PredictionInput input, bool ft, bool checkCollision)
            {
                ...
                if (float.MaxValue - input.Radius > float.Epsilon &&
                    input.RangeCheckFrom.Distance(input.Unit.Position) > input.Range * 3f)
                {
                    return new PredictionOutput { Input = input };
                }
                    ...
                    if (input.RangeCheckFrom.DistanceSquared(result.CastPosition) > Math.Pow(input.Range, 2))
                    {
                        if (result.Hitchance != HitChance.OutOfRange)
                        {
                            result.CastPosition = input.RangeCheckFrom +
                                                  input.Range *
                                                  (result.UnitPosition - input.RangeCheckFrom).ToVector2().Normalized()
                                                  .ToVector3();
                        }
                        else
                        {
            ...
            internal static PredictionOutput WayPointAnalysis(PredictionOutput result, PredictionInput input)
            {
                ...
                if (distanceUnitToWaypoint > 0)
                {
                    ...
                    if (getAngle < 20 || getAngle > 160 || ((getAngle > 150 || getAngle < 30) &&
                                                            OktwCommon.IsMovingInSameDirection(ObjectManager.Player,
                                                                input.Unit)))
                    {
            ...
            internal static PredictionOutput GetDashingPrediction(PredictionInput input)
            {
                ...
                if (dashData != null && !(dashData.Speed > 10000))
                {
                    ...
                    if (dashPred.Hitchance >= HitChance.High && dashPred.UnitPosition.ToVector2()
                            .Distance(input.Unit.Position.ToVector2(), endP, true) < 200)
                    {
                    ...
                    {
                        var timeToPoint = input.Delay / 2f + input.From.ToVector2().Distance(endP) / input.Speed -
                                          0.25f;
                        if (timeToPoint <=
                ...
                var result =
                    unit.Buffs.Where(
                            buff =>
                                ...
                                buff.Type == BuffType.Asleep)
                        .Aggregate(0d, (current, buff) => Math.Max(current, buff.EndTime));
                return (result - Game.Time);
            }

            internal static PredictionOutput GetPositionOnPath(PredictionInput input, List<Vector2> path,
                float speed = -1)
            {
                ...
                if (pLength >= tDistance && Math.Abs(input.Speed - float.MaxValue) > float.Epsilon)
                {
                    ...
                    for (var i = 0; i < path.Count - 1; i++)
                    {
                        ...
                        var sol = Geometry.VectorMovementCollision(a, b, speed, input.From.ToVector2(), input.Speed,
                            tT);
                        var t = (float)sol[0];
            ...
            public static class Circle
            {
                public static PredictionOutput GetPrediction(PredictionInput input)
                {
                    ...
                    while (posibleTargets.Count > 1)
                    {
                        ...
                        {
                            var distance =
                                Vector2.DistanceSquared(posibleTargets[i].Position, posibleTargets[0].Position);
                ...
                public static PredictionOutput GetPrediction(PredictionInput input)
                {
                    ...
                    if (posibleTargets.Count > 1)
                    {
                        ...
                        foreach (var candidate in candidates)
                        {
                            if (
                                GetHits(
                                    input.From.ToVector2(), candidate,
                            ...
                            case CollisionableObjects.Minions:
                                foreach (var minion in Cache.GetMinions(input.From, Math.Max(input.Range + 500, 2000)))
                                {
...

                                    {
                                        ...
                                        if (minionPos.ToVector2().Distance(input.From.ToVector2(), position.ToVector2(),
                                                false) <= input.Radius + bonusRadius + minion.BoundingRadius)
                                        {
                                            return true;
                                        }
                                    }
        ...
        internal static class UnitTracker
        {
            ...
            static UnitTracker()
            {
                ...
                foreach (var hero in ObjectManager.Get<AIHeroClient>())
                {
                    ...
                    {
                        NetworkId = (int)hero.NetworkId, AaTick = Variables.TickCount,
                        StopMoveTick = Variables.TickCount, NewPathTick = Variables.TickCount,
                        SpecialSpellFinishTick = Variables.TickCount, LastInvisableTick = Variables.TickCount
                    });
                }
            ...
            private static void AIBaseClient_OnEnterLocalVisiblityClient(AttackableUnit sender, EventArgs args)
            {
                if (sender is AIHeroClient)
                    UnitTrackerInfoList.Find(x => x.NetworkId == sender.NetworkId).LastInvisableTick =
                        Variables.TickCount;
            }
            ...
            private static void AIHeroClient_OnNewPath(AIBaseClient sender, AIBaseClientNewPathEventArgs args)
            {
                if (sender is AIHeroClient)
                {
                    ...
                    item.PathBank.Add(new PathInfo()
                        { Position = args.Path.Last().ToVector2(), Time = Variables.TickCount });
                    ...
                    else
                    {
                        ...
                        if (foundSpell != null)
                        {
                            UnitTrackerInfoList.Find(x => x.NetworkId == sender.NetworkId).SpecialSpellFinishTick =
                                Variables.TickCount + (int)(foundSpell.duration * 1000);
                        }
                        else if (sender.IsWindingUp || !sender.CanMove)
                        {
                            UnitTrackerInfoList.Find(x => x.NetworkId == sender.NetworkId).SpecialSpellFinishTick =
                                Variables.TickCount + 100;
                        }
                    }
                }
            }
            ...
            public static bool SpamSamePlace(AIBaseClient unit)
            {
                ...
                if (TrackerUnit.PathBank[2].Time - TrackerUnit.PathBank[1].Time < 180 &&
                    Variables.TickCount - TrackerUnit.PathBank[2].Time < 90)
                {
