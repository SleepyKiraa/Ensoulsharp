...
namespace SPredictionMash1
{
    ...
    public static class Prediction
    {
        ...
        {
            return GetPrediction(target, width, delay, missileSpeed, range, collisionable, type, target.GetWaypoints(), target.AvgMovChangeTime(), target.LastMovChangeTime(), target.AvgPathLenght(), target.LastAngleDiff(), ObjectManager.Player.ServerPosition.ToVector2(), ObjectManager.Player.ServerPosition.ToVector2());
        }
        ...
        internal static Result GetPrediction(AIBaseClient target, float width, float delay, float missileSpeed, float range, bool collisionable, SpellType type, List<Vector2> path, float avgt, float movt, float avgp, float anglediff, Vector2 from, Vector2 rangeCheckFrom)
        {
            ...
            try
            {
                ...
                if (target is AIHeroClient)
                {
                    ...
                    {
                        if (target.AttackCastDelay * 1000 + PathTracker.EnemyInfo[(uint)target.NetworkId].AvgOrbwalkTime + avgt - width / 2f / target.MoveSpeed >= GetArrivalTime(target.ServerPosition.ToVector2().Distance(from), delay, missileSpeed))
                        {
                ...
                float d = result.CastPosition.Distance(target.ServerPosition.ToVector2());
            ...
            if (pathBounds[0] != -1 && pathBounds[1] != -1)
            {
                for (int k = pathBounds[0]; k <= pathBounds[1]; k++)
                {
                    ...
                    for (int i = 1; i < steps - 1; i++)
                    {
                        ...
                        Vector2 currentPosition = target.ServerPosition.ToVector2();
        ...
        public static Vector2 GetFastUnitPosition(AIBaseClient target, float delay, float missileSpeed = 0, Vector2? from = null, float distanceSet = 0)
        {
            ...
            if (distance == 0)
            {
                ...
                if (missileSpeed != 0 && path.Count == 2)
                {
                    ...
                    Vector2 Vs = (target.ServerPosition.ToVector2() - from.Value).Normalized() * missileSpeed;
                    Vector2 Vr = Vt - Vs;
