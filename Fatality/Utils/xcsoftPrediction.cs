...
namespace Fatality.Utils
{
    ...
    {
        public static float PredHealth(AIBaseClient Target, Spell spell) => HealthPrediction.GetPrediction(Target, (int)(ObjectManager.Player.Distance(Target) / spell.Speed), (int)(spell.Delay * 1000 + Game.Ping / 2));

        public static void CastCircle(this Spell spell, AIBaseClient target)
        {
            if (spell.Type == SpellType.Circle || spell.Type == SpellType.Cone)
            {
                if (target == null)
                {
                    return;
