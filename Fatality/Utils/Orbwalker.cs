...
namespace Fatality.Utils
{
	internal class NewOrbwalker : IOrbwalker
    {
		...
		public NewOrbwalker()
        {
			...
			AttackEnabled = true;
			MoveEnabled = true;
			Menu = new Menu("Orbwalker", "[Fatality] Orbwalker", true).Attach();
			...
			OrbwalkerMenu.Add(new MenuBool("HighOrb", "High AS Mode", false));
			OrbwalkerMenu.Add(new MenuBool("CalculateRunaway", "Calculate Run away time in Orb limit distance", true));
			FarmMenu = Menu.Add(new Menu("Farm", "Farm", false));
			FarmMenu.Add(new MenuSlider("FarmDelay", "Farm Delay", 30, 0, 200));
			FarmMenu.Add(new MenuSlider("FastFarmDelay", "Fast Farm Delay", 220, 0, 1000));
			...
			DrawMenu.Add(new MenuBool("DrawChaseRange", "Draw Force Chase Range", true));
			DrawMenu.Add(new MenuBool("ShowFakeClick", "Show FakeClick", false));
			MiscMenu = Menu.Add(new Menu("Misc", "Extra Range Setting"));
			MiscMenu.Add(new MenuSlider("Range", "Extra LowHP Target Find range", 200, 0, 500));
			MiscMenu.Add(new MenuKeyBind("FindKey", "Enable Force chase Mode(Combo Activating)", Keys.LButton, KeyBindType.Press));
			...
			Menu.Add(new MenuKeyBind("FastLaneClear", "Fast LaneClear", Keys.LButton, KeyBindType.Press, false));
		...
		private void OnDraw(EventArgs args)
		{
			...
			if (GameObjects.Player.Position.IsValid())
			{
                ...
                if (FastLne)
                {
					Drawing.DrawText(Drawing.WorldToScreen(Game.CursorPos), System.Drawing.Color.White, "Fast Farm Mode");
                }
				if (ForceChase)
				{
					Drawing.DrawText(Drawing.WorldToScreen(Game.CursorPos), System.Drawing.Color.White, "Force Chase Mode");
                    ...
                    if (DrawMenu["DrawChaseRange"].GetValue<MenuBool>().Enabled)
                    {
						...
						if (colorindex >= 450)
							colorindex = 0;
						var colorm = Fatality.Utils.Base.PlusRender.GetFullColorList(450);
